...
> **⚠️ 本次重构遵循"零修改"原则：**
> - 不得修改 `source/` 目录下的任何原有代码
> - 不得修改 `tests/` 目录下的任何测试代码
> - 所有重构工作仅在 `refactor/` 目录下进行
> - 重构完成后，通过配置或入口文件切换新旧版本
> - 原项目代码保持不变，作为备份和对照参考
...
```
source/core/           → 复制引用 (不修改)
source/services/      → 复制引用 (不修改)
source/i18n/          → 复制引用 (轻微修改)
refactor/gui_qt/      → 新建
```
